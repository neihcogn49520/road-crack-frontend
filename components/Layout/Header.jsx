    ...
    <>
      <header className="sticky z-30 top-0 w-screen font-family">
        <nav className="border-b-2 border-text-main/5 h-[60px] lg:h-[70px] w-full flex flex-col justify-center items-center bg-background-paper">
          <div className="max-w-[1140px] flex w-full flex-grow items-center px-2">
            <div className="flex justify-between w-full md:w-auto flex-grow md:flex-grow-0 items-center transition-all">
              <div className="lg:hidden cursor-pointer mr-5" onClick={handleDrawerOpen}>
                <HiMenuAlt1 className="w-[24px] h-[24px] " />
              </div>
              <div
                onClick={() => {
                  try {
                    document.getElementById("#banner").scrollIntoView({ block: "start", behavior: "smooth" })
                  } catch (error) {
                    router.push("/")
                  }
                }}
                className="text-primary-main w-full justify-center md:justify-start md:w-auto font-bold text-xl uppercase cursor-pointer flex gap-2 items-center"
              >
                <Image
                  width={500}
                  height={500}
            ...
            <ul className="mx-auto hidden lg:flex lg:px-4">
              {navigations.map(({ name, url }, index) => (
                <li key={index} className="h-[60px] lg:h-[70px] flex px-2 justify-center items-center font-semibold">
                  <Link href={url}>
                    <div
                    ...
                    >
                      <span className={router.asPath == url ? "!text-main-main" : ""}>{name}</span>
                    </div>
                  </Link>
                </li>
              ))}
            ...
            <div className=" space-x-1">
              <Link href="/login">
                <button
                  className={`hidden lg:inline-flex ml-auto capitalize items-center cursor-pointer group align-middle text-center transition-all select-none whitespace-nowrap py-2 px-4 text-sm leading-normal no-underline rounded-full  dark:hover:bg-primary-main hover:bg-primary-main text-main dark:border-text-main/10 outline-none font-bold ${
      ...
      >
        <ul className="flex flex-col w-full items-start h-full px-8 py-6 text-base ">
          <div className="flex-1 flex flex-col items-start">
            {[...navigations, ...mobileMoreItems].map(({ name, url }, index) => (
              <li key={index} className="py-2 text-text-main font-semibold" onClick={handleDrawerClose}>
                <Link href={url}>
                  <div className="flex justify-center items-center hover:text-main-main transition-all">
                    <span className={router.asPath == url ? "!text-main-main !font-extrabold" : ""}>{name}</span>
                  </div>
                </Link>
              </li>
            ))}
        ...
        <div className="absolute top-5 right-5 cursor-pointer block" onClick={handleDrawerClose}>
