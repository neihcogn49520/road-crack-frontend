          ...
          <div className="skill-list">
            ...
            <ul className="department-service">
              {experties.map((content, index) => (
                <li key={index} className="flex items-center py-1 md:py-1.5 opacity-90">
          ...
          <ul className="list-unstyled text-lg md:text-base md:opacity-90">
            {Object.keys(timetable).map((key, index) => (
              <li key={key} className="flex justify-between items-center">
                <span className="opacity-90 text-sm sm:text-base font-medium">{key}</span>
                <span className="opacity-90 text-sm sm:text-base ">{timetable[key]}</span>
              </li>
            ))}
          </ul>
          <div className="mt-8">
            <h6 className="text-lg md:text-xl tracking-wider !leading-tight mb-3">Need Urgent Help?</h6>
            <p className=" opacity-100"> Call Us On <span className="text-main-main font-bold"> +91 00000 00000</span></p>
          </div>
        </div>
      </div>
    </>
